    ...
    #[cfg(target_family = "wasm")]
    {
        ...
        loop {
            ...
            match stream.capture_uvc_frame() {
                Ok(f) => {
                    frame_idx += 1;
                    // MJPEG frames from UVC webcams are already valid JPEG bytes
                    // (SOI FF D8 .. EOI FF D9), so a plain write is enough.
                    // The host preopens "." (current dir), so we write a relative path.
                    // Run the host from a dir that contains an "out/" subdirectory:
                    //   mkdir -p out && sudo usb-wasi-host -c webcam.wasm
                    if let Err(e) = std::fs::write("out/latest.jpg", &f.data) {
