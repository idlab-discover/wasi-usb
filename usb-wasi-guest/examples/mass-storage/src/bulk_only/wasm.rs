...
impl BulkOnlyTransportDevice {
    ...
    pub fn new(
        ...
        let lun_data = await_transfer(&xfer).map(|r| r.data).unwrap_or(vec![0]);
    ...
    pub fn command_in(
        ...
        let data = if transfer_length > 0 {
            ...
            await_transfer(&xfer_data).map(|r| r.data).unwrap_or_default()
        } else {
            vec![]
        };
        ...
        let csw_bytes = await_transfer(&xfer_csw).map(|r| r.data).unwrap_or_default();
